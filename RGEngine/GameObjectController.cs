...
namespace RGEngine
{
    public class GameObjectController : gameController
    {
        ...
        protected override void Update()
        {
            ...
            _gol.Where(go => go.Enabled).Select(go => go._update());
        }
        protected override void GUI()
        {
            //LINQ, Where go.Enabled, select and run its _gui
            _gol.Where(go => go.Enabled).Select(go => go._gui());
        }
        protected override void Render()
        {
            //LINQ, Where go.Enabled, select and run its _render
            _gol.Where(go => go.Enabled).Select(go => go._render());
        }
    }
}
